...
const projectInfo = [
   {
    ...
    A: "基于Vue 3、TypeScript、Vite与Pinia构建前端应用，接入集团微前端框架；完成分屏录像回放模块开发，结合Canvas时间轴实现录像片段展示、播放进度联动与时间定位，通过requestAnimationFrame优化高频交互与绘制；封装可拖拽录像计划组件，实现时间段缩放、网格吸附及边界约束，支持模板复用；封装WebSocket通信模块，实现心跳保活、指数退避重连与消息分发；结合路由懒加载、组件资源清理及四语言适配，完善应用性能与可维护性。",
    R: "完成六大业务模块建设，形成从存储配置、录像计划到回放检索与下载的完整管理流程，支持1/4/9分屏回放及中、英、越、印尼四种语言；沉淀时间模板、回放时间轴与通信等可复用模块，为后续功能迭代和多语言交付提供基础。使用 AI 辅助完成页面开发，将页面开发时间缩短约 30%。",
  },
  {
    companyName: "润和软件(中冶赛迪)",
    projectName: "主数据平台",
